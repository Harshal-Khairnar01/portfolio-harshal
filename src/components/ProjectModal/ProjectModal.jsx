...
export default function ProjectModal({ project, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black  bg-opacity-80 flex justify-center items-center z-50  animate-fade-in">
      <div className=" bg-[#484747]  bg-opacity-35 w-[90%] max-w-2xl p-5 rounded-2xl relative text-white border-2 border-gray-500 animate-slide-up">
        <button
          onClick={onClose}
        ...
        <div className=" mt-4">
          <Image
            ...
            height={800}
            placeholder="blur"
            blurDataURL={`/project${project.img}`} // reuse same image as low-res placeholder
            className="rounded-md w-full object-cover transition-opacity duration-500"
          />

          <h1 className={`text-3xl font-bold mt-2 ${styles.head}`}>
            {project.heading}
          </h1>
          <p className={`text-gray-300 mt-2 text-sm ${styles.changa}`}>
            {project.desc}
          </p>
